...
const OPENROUTER_MODEL_MAP = {
  ...
  'meta-llama/llama-4-scout-17b-16e-instruct': 'deepseek/deepseek-chat-v3-0324',
}

function mapModelToOpenRouter(groqModel) {
  return OPENROUTER_MODEL_MAP[groqModel] || 'deepseek/deepseek-chat-v3-0324'
}
...
async function openrouterChat(params) {
  ...
  // Build the request body — strip Groq-specific tools/tool_choice
  const { tools, tool_choice, ...rest } = params
  
  // Truncate long system prompts to save tokens on OpenRouter (credit-limited)
  const messages = (rest.messages || []).map(m => {
    if (m.role === 'system' && m.content && m.content.length > 8000) {
      return { ...m, content: m.content.slice(0, 8000) + '\n\n[...system prompt truncated for fallback provider...]' }
    }
    return m
  })
  
  // Keep only last 6 messages to reduce token count
  const trimmedMessages = messages.length > 7 ? [messages[0], ...messages.slice(-6)] : messages
  
  const body = { ...rest, messages: trimmedMessages, model: orModel, max_tokens: rest.max_tokens || 2048 }
...
async function geminiChat(params) {
  ...
  // Build the request body — strip tools/tool_choice (not supported by Gemini in same format)
  const { tools, tool_choice, ...rest } = params
  const body = { ...rest, model: geminiModel }
...
export async function groqChat(params) {
  ...
  try {
    ...
    const status = err.status || err.statusCode || 0
    const msg = err.message || ''
    console.error('[groq] Primary failed', { status, msg: msg.slice(0, 200), errKeys: Object.keys(err).join(',') })
    const isRetryable = isGroqRateLimitError(err) || (status >= 500 && status < 600) || status === 403 || status === 401
    ...
    if (fallback) {
      ...
      try {
        ...
        const fbRetryable = isGroqRateLimitError(fallbackErr) || (fbStatus >= 500 && fbStatus < 600) || fbStatus === 403 || fbStatus === 401
        if (!fbRetryable) {
