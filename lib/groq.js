...
export async function groqChat(params) {
  ...
  for (const p of providers) {
    ...
    try {
      ...
      const status = err.status || err.statusCode || 0
      const isRetryable = isRateLimitError(err) || (status >= 500 && status < 600)
  ...
  if (!anyProviderAvailable) {
    throw new GroqError('No LLM providers configured', 'GROQ_NOT_CONFIGURED')
  }

  // If any provider hit a rate limit, surface that as the user-facing error
  // (more actionable than a generic 500).
  if (lastRateLimitErr) {
    throw new GroqError('Too many requests, try again later.', 'GROQ_QUOTA_EXCEEDED', lastRateLimitErr)
  }
  throw new GroqError(lastErr?.message || 'All providers failed.', 'GROQ_SERVER_ERROR', lastErr)
}
