...
export async function groqChat(params) {
  ...
  const providers = [
    ...
    },
    {
      ...
      //
      // IMPORTANT: 8B has a strict 6K TPM limit. The Kivora system prompt +
      // 28-tool array is ~8K tokens, which exceeds 6K. So this provider only
      // fires when tools are NOT being sent (no-tools requests) or when the
      // caller has explicitly stripped the system prompt. When tools are on,
      // skip — we'd just get a 413 "Request too large" error.
      name: 'groq-fast',
      available: () => {
        if (!getPrimaryClient() && !_groqPrimary) return false
        // Skip when tools are present (input too large for 6K TPM)
