...
//
// NOTE: The @e2b/code-interpreter package uses Node.js APIs (node:path, etc.)
// that are incompatible with edge runtime / Cloudflare Workers bundling.
// It's listed in serverExternalPackages and is NOT bundled at build time.
// It's loaded dynamically at runtime only when E2B_API_KEY is set.
// The package must be installed on the deployment server for runtime use.
...
async function getSDK() {
  ...
  // Use eval-based dynamic import to prevent esbuild/webpack from resolving at build time
  // The E2B SDK uses Node.js APIs incompatible with edge runtime bundlers
  const mod = await Function('return import("@e2b/code-interpreter")')()
