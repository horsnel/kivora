    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          platform: { type: 'string', description: 'Force a specific platform: auto (default), kaggle, colab, e2b, or codespaces.', enum: ['auto', 'kaggle', 'colab', 'e2b', 'codespaces'], default: 'auto' },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          platform: { type: 'string', description: 'Force a specific platform: auto (default), e2b, kaggle, colab, or codespaces.', enum: ['auto', 'e2b', 'kaggle', 'colab', 'codespaces'], default: 'auto' },
...
`
  ...
  async deploy_site(args) {
    const CF_API_TOKEN = process.env.CF_API_TOKEN || ''
    const CF_ACCOUNT_ID = process.env.CF_ACCOUNT_ID || ''

    if (!CF_API_TOKEN) return JSON.stringify({ success: false, error: 'Cloudflare API token not configured. Set CF_API_TOKEN env var.' })
    if (!CF_ACCOUNT_ID) return JSON.stringify({ success: false, error: 'Cloudflare Account ID not configured. Set CF_ACCOUNT_ID env var.' })
    ...
    try {
      // First, try to create the project (will fail if it already exists, that's fine)
      const createRes = await fetch(
        `https://api.cloudflare.com/client/v4/accounts/${CF_ACCOUNT_ID}/pages/projects`,
        {
          ...
          body: JSON.stringify({
            ...
            build_config: args.framework && args.framework !== 'static'
              ? { framework: args.framework, build_command: 'npm run build', output_directory: 'dist' }
              : undefined,
          }),
        }
      )
      // Don't check createRes status — project may already exist

      // Now deploy files using the direct upload API
