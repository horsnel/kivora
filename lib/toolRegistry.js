...
// Supports Cloudflare Sandbox (primary) with Judge0 (fallback)
...
const toolDefs = [
  ...
  // ── 21. SANDBOX EXEC ── (Cloudflare Sandbox only)
  {
    type: 'function',
    function: {
      name: 'sandbox_exec',
      description: 'Execute a shell command in a persistent Cloudflare Sandbox with full internet access, pip/npm install, and filesystem. Use for installing packages, running servers, git operations, or any command-line task. Only available when sandbox is configured.',
      parameters: {
        type: 'object',
        properties: {
  ...
  // ── 22. SANDBOX FILE OPERATION ──
  {
    ...
    function: {
      name: 'sandbox_file',
      description: 'Read, write, list, or manage files in the Cloudflare Sandbox filesystem. Use to create downloadable files (PDFs, CSVs, images, etc.), read project files, or organize a workspace.',
      parameters: {
        type: 'object',
        properties: {
  ...
  // ── 23. SANDBOX GIT ──
  {
    ...
    function: {
      name: 'sandbox_git',
      description: 'Clone a git repository into the Cloudflare Sandbox for analysis, modification, or building. Use when the user wants to analyze, explore, or work with a GitHub repo.',
      parameters: {
        type: 'object',
        properties: {
  ...
  // ── 26. RUN ON GPU ──
  {
    ...
    function: {
      name: 'run_on_gpu',
      description: 'Execute Python code on a remote GPU or TPU runtime. Use when the user needs GPU-accelerated computing — ML training, model fine-tuning, deep learning inference, CUDA operations, large-scale data processing, or any compute-heavy task. Automatically selects the best available backend (Google Colab or Kaggle Kernels). Colab supports T4/L4/A100/H100 GPUs and v5e1/v6e1/V2-8 TPUs. Kaggle supports T4/P100 GPUs and TPU v3-8. Both offer free-tier GPU access.',
      parameters: {
        type: 'object',
        properties: {
          code: { type: 'string', description: 'Complete Python code to execute on the GPU/TPU runtime. Include all imports. Code runs in an isolated notebook environment.' },
          ...
          tpu: { type: 'string', description: 'TPU type: v5e1, v6e1, V2-8 (Colab) or v3-8 (Kaggle). Use instead of GPU for TPU workloads.', enum: ['v5e1', 'v6e1', 'V2-8', 'v3-8'] },
          platform: { type: 'string', description: 'Execution platform: "auto" (default, picks best available), "colab", or "kaggle". Use "kaggle" for free T4/P100 GPU without Google OAuth setup.', enum: ['auto', 'colab', 'kaggle'], default: 'auto' },
...
`
  ...
  async sandbox_file(args) {
    if (!SANDBOX_AVAILABLE) {
      return JSON.stringify({ error: 'Cloudflare Sandbox not configured. File operations require the sandbox service.' })
    }
    try {
      const sandboxId = args.sandbox_id || 'default'
      ...
      if (action === 'ls') {
        // Use the DO ls endpoint directly
      ...
      if (action === 'mkdir') {
        // Create a directory marker in DO storage
  ...
  async sandbox_git(args) {
    if (!SANDBOX_AVAILABLE) {
      return JSON.stringify({ error: 'Cloudflare Sandbox not configured. Git operations require the sandbox service.' })
    }
    try {
      const sandboxId = args.sandbox_id || 'default'
      ...
      // Use the sandbox worker's exec which proxies to Judge0
      // Git clone requires a persistent filesystem — note the limitation
      const result = await sandboxExec(sandboxId, `git clone --depth 1${branch} ${repoUrl} /workspace/${targetDir}`, 60000)
      if (result.success) {
  ...
  async run_on_gpu(args) {
    ...
    const timeout = Math.min((args.timeout || 120) * 1000, 300000)
    const installPackages = args.install_packages
      ? args.install_packages.split(',').map(p => p.trim()).filter(Boolean)
      : []

    // Prepend pip installs to code if needed
    let code = args.code
    if (installPackages.length > 0) {
      const installCode = `import subprocess; subprocess.check_call(['pip', 'install', '-q'] + ${JSON.stringify(installPackages)})\n`
      code = installCode + code
    }

    // ── Determine which backend to use ──
    let colabAvailable = false
    let kaggleAvailable = false

    try {
      const colabModule = await import('@/lib/colab')
      if (colabModule.isColabAvailable()) colabAvailable = true
    } catch {}

    try {
      const kaggleModule = await import('@/lib/kaggle')
      if (kaggleModule.isKaggleAvailable()) kaggleAvailable = true
    } catch {}

    // ── Auto-select or enforce platform ──
    let useBackend = null
    if (platform === 'colab') {
      if (!colabAvailable) {
        return JSON.stringify({
          error: 'Google Colab not available. Set GOOGLE_COLAB_ACCESS_TOKEN env var, or use platform "kaggle" which only needs an API key.',
          fallback: 'cpu',
        })
      }
      useBackend = 'colab'
      ...
      if (!kaggleAvailable) {
        return JSON.stringify({
          error: 'Kaggle not available. Set KAGGLE_USERNAME and KAGGLE_KEY env vars. Get your API key at https://www.kaggle.com/settings → API → Create New Token.',
          fallback: 'cpu',
        })
      }
      useBackend = 'kaggle'
    } else {
      // Auto: prefer Kaggle (simpler setup, no OAuth), fall back to Colab
      if (kaggleAvailable) useBackend = 'kaggle'
      else if (colabAvailable) useBackend = 'colab'
    }

    if (!useBackend) {
      return JSON.stringify({
        error: 'No GPU platform available. Set KAGGLE_USERNAME + KAGGLE_KEY (easiest, free T4 GPU) or GOOGLE_COLAB_ACCESS_TOKEN in your environment.',
        fallback: 'cpu',
        suggestion: 'For quick setup: go to https://www.kaggle.com/settings → API → Create New Token. Add KAGGLE_USERNAME and KAGGLE_KEY to your environment.',
      })
    }

    // ── Execute on selected backend ──
    try {
      if (useBackend === 'kaggle') {
        const kaggleModule = await import('@/lib/kaggle')
        const result = await kaggleModule.runKaggleGpuJob(code, {
          gpu: args.tpu ? undefined : (args.gpu || 'T4'),
          tpu: args.tpu === 'v3-8' ? 'v3-8' : undefined,
          installPackages,
        ...
        return JSON.stringify({
          ...
          files: result.files || [],
          sessionUsed: result.sessionUsed || 'ephemeral',
          kernelUrl: result.kernelUrl || null,
        })
      }

      if (useBackend === 'colab') {
        const colabModule = await import('@/lib/colab')
        const gpu = args.tpu ? undefined : (args.gpu || 'T4')
        const tpu = args.tpu || undefined

        const result = await colabModule.runGpuJob(code, { gpu, tpu, timeout })
        ...
        return JSON.stringify({
          ...
          accelerator: result.accelerator || gpu || tpu || 'T4',
          files: result.files || [],
          sessionUsed: result.sessionName || 'ephemeral',
        })
      }
    } catch (err) {
      // If the selected backend fails, try the other one (auto-fallback)
      if (platform === 'auto') {
        const fallbackBackend = useBackend === 'kaggle' ? 'colab' : 'kaggle'
        const fallbackAvailable = fallbackBackend === 'kaggle' ? kaggleAvailable : colabAvailable

        if (fallbackAvailable) {
          try {
            if (fallbackBackend === 'kaggle') {
              const kaggleModule = await import('@/lib/kaggle')
              const result = await kaggleModule.runKaggleGpuJob(code, {
                gpu: args.gpu || 'T4',
                installPackages,
              ...
              return JSON.stringify({
                ...
                error: result.error || null,
                executor: 'kaggle',
                accelerator: result.accelerator || 'T4',
                acceleratorName: result.acceleratorName || 'NVIDIA T4 (Free)',
                files: result.files || [],
                sessionUsed: result.sessionUsed || 'ephemeral',
                note: `Fallback: ${useBackend} failed (${err.message}), used kaggle instead`,
              })
            } else {
              const colabModule = await import('@/lib/colab')
              const result = await colabModule.runGpuJob(code, { gpu: args.gpu || 'T4', timeout })
              return JSON.stringify({
                success: result.success,
                output: result.output || '(no output)',
                error: result.error || null,
                ...
                files: result.files || [],
                sessionUsed: result.sessionName || 'ephemeral',
                note: `Fallback: ${useBackend} failed (${err.message}), used colab instead`,
              })
            }
          } catch (fallbackErr) {
            return JSON.stringify({
              error: `Both backends failed. ${useBackend}: ${err.message}. ${fallbackBackend}: ${fallbackErr.message}. Try execute_code for CPU execution.`,
              fallback: 'cpu',
            })
          }
        }
...
QUICK REFERENCE — Your 26 tools:
Search: web_search, search_wikipedia, search_news, search_images
Code: execute_code, calculate_math, format_json, run_on_gpu
...
Artifact types: html (web pages/dashboards/tools), svg (graphics/icons), mermaid (diagrams/flowcharts), markdown (documents/reports), react (interactive components)
Rules: Complete code only. HTML needs <!DOCTYPE html>, full <head>, <body>, inline styles/scripts. Responsive by default. No external deps except CDN. Maps use Leaflet+OSM in HTML artifact.
...
GPU/TPU EXECUTION:
- run_on_gpu: Execute Python code on a remote GPU or TPU runtime (Colab or Kaggle)
- Use when the user needs GPU-accelerated computing: ML training, model fine-tuning, deep learning inference, CUDA operations, large-scale data processing
- Platform auto-selects: Kaggle (simpler, API key only) preferred, falls back to Colab
- User can specify platform: "kaggle" for easy setup, "colab" for more GPU options
- Kaggle: Free T4/P100 GPUs, TPU v3-8. Needs KAGGLE_USERNAME + KAGGLE_KEY env vars
- Colab: T4/L4/A100/H100 GPUs, v5e1/v6e1/V2-8 TPUs. Needs GOOGLE_COLAB_ACCESS_TOKEN
- If neither is available, tool returns fallback suggestion. Suggest Kaggle as easiest to set up
- For CPU-only Python tasks, prefer execute_code instead of run_on_gpu
- Use install_packages parameter for any pip dependencies needed (e.g. "transformers,datasets,peft")
- Max timeout is 300 seconds; default is 120 seconds
- Auto-fallback: if primary backend fails, tries the other before giving up`
