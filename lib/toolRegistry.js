    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          platform: { type: 'string', description: 'Force a specific platform: auto (default), daytona, kaggle, colab, e2b, or codespaces.', enum: ['auto', 'daytona', 'kaggle', 'colab', 'e2b', 'codespaces'], default: 'auto' },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          platform: { type: 'string', description: 'Force a specific platform: auto (default), daytona, e2b, kaggle, colab, or codespaces.', enum: ['auto', 'daytona', 'e2b', 'kaggle', 'colab', 'codespaces'], default: 'auto' },
...
`
  ...
  async sandbox_exec(args) {
    ...
    if (DAYTONA_AVAILABLE) {
      try {
        ...
        // Daytona failed — fall through to Cloudflare Sandbox
      } catch {
        // Daytona error — fall through
      }
    }

    // 2. Try Cloudflare Sandbox
    if (SANDBOX_AVAILABLE) {
      try {
  ...
  async sandbox_file(args) {
    ...
    if (DAYTONA_AVAILABLE) {
      try {
        ...
        // Daytona error — fall through to Cloudflare Sandbox
      }
    }
    ...
    return JSON.stringify({ error: 'No sandbox provider configured. Set DAYTONA_API_KEY or KIVORA_SANDBOX_URL for file operations.' })
  },
  ...
  async sandbox_git(args) {
    ...
    // 3. No sandbox available
    return JSON.stringify({ error: 'No sandbox provider configured. Set DAYTONA_API_KEY or KIVORA_SANDBOX_URL for git operations.' })
  },
