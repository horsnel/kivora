...
const toolDefs = [
  ...
  // ── 26. RUN ON GPU ──
  {
    ...
    function: {
      name: 'run_on_gpu',
      description: 'Execute Python code on a Google Colab GPU or TPU runtime. Use when the user needs GPU-accelerated computing — ML training, model fine-tuning, deep learning inference, CUDA operations, large-scale data processing, or any compute-heavy task that requires GPU/TPU. Automatically provisions a session, runs the code, and releases the GPU. Supports T4, L4, A100, H100 GPUs and v5e1, v6e1, V2-8 TPUs.',
      parameters: {
        type: 'object',
        properties: {
          code: { type: 'string', description: 'Complete Python code to execute on the GPU/TPU runtime. Include all imports. The code runs in /content directory on a Colab VM.' },
          gpu: { type: 'string', description: 'GPU type: T4 (free, 16GB), L4 (24GB), A100 (40/80GB), H100 (80GB, latest). Default: T4', enum: ['T4', 'L4', 'A100', 'H100'], default: 'T4' },
          tpu: { type: 'string', description: 'TPU type: v5e1, v6e1, V2-8. Use instead of GPU for TPU workloads.', enum: ['v5e1', 'v6e1', 'V2-8'] },
...
`
  ...
  async run_on_gpu(args) {
    try {
      ...
      const { setColabAccessToken, runGpuJob, isColabAvailable } = colabModule

      // Try to get access token from environment
      // Note: In the chat API, the token would be set via setColabAccessToken before tool handlers run
      if (!isColabAvailable()) {
        return JSON.stringify({
          error: 'Google Colab not connected. The user needs to connect their Google account first (visit /colab to set up). Falling back to CPU execution.',
          fallback: 'cpu',
          suggestion: 'Connect your Google account at kivora.pages.dev/colab to enable GPU/TPU execution.',
        })
      }
      ...
      if (args.install_packages) {
        const pkgs = args.install_packages.split(',').map(p => p.trim()).filter(Boolean)
        // Packages will be installed as part of the code
        const installCode = `import subprocess; subprocess.check_call(['pip', 'install', '-q'] + ${JSON.stringify(pkgs)})\n`
        args.code = installCode + args.code
      }

      const result = await runGpuJob(args.code, { gpu, tpu, timeout })
      ...
      return JSON.stringify({
        error: `GPU execution unavailable: ${err.message}. Try using execute_code for CPU execution instead.`,
        fallback: 'cpu',
      })
    }
  }
...
GPU/TPU EXECUTION:
- run_on_gpu: Execute Python code on a Google Colab GPU or TPU runtime
...
- Defaults to T4 GPU (free tier). Supports L4, A100, H100 GPUs and v5e1, v6e1, V2-8 TPUs
- If Colab is not connected, the tool returns a fallback suggestion. Inform the user they can connect at /colab
