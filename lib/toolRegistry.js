...
function filterToolsByQuery(query) {
  ...
  const categories = [
    ...
    },
    {
      ...
      keywords: ['image', 'picture', 'photo', 'draw', 'generate', 'create', 'design', 'logo', 'icon', 'art', 'illustration', 'qr', 'barcode', 'color', 'palette', 'paint'],
    },
    {
  ...
  // If query is short and trivial (greeting, thanks, simple recall), include
  // only web_search — the model doesn't need anything else.
  const isTrivial = q.length < 20 || /^(hi|hey|hello|yo|sup|thanks|thank you|thx|ok|okay|cool|nice|great|bye|goodbye|yes|no|sure|lol|haha)\b/i.test(q)
  if (isTrivial) {
  ...
  if (filtered.length < 3) {
    const defaultSet = new Set(['web_search', 'execute_code', 'search_wikipedia', 'read_url'])
