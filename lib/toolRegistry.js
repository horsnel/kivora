...
const toolHandlers = {
  ...
  async generate_image(args) {
    ...
    try {
      const imageUrl = 'https://image.pollinations.ai/prompt/' + encodeURIComponent(prompt) + `?width=${Math.min(w, 1024)}&height=${Math.min(h, 1024)}&nologo=true&model=flux`
...
/**
 * Detect when the user's query CLEARLY requires a specific tool.
 * Returns the tool name that must be called, or null if no specific tool
 * is mandated by the query.
 *
 * This is used to set tool_choice to force the model to call that tool
 * (instead of 'auto' which lets the model skip the tool and describe
 * what it would do — a common failure mode for Qwen 2.5 and Llama 3.3).
 */
function detectRequiredTool(query) {
  if (!query || typeof query !== 'string') return null
  const q = query.toLowerCase().trim()

  // ── Image generation ──
  // "generate an image of", "draw a", "create a picture", "make an illustration"
  // These phrases unambiguously mean the user wants an image generated.
  const imagePatterns = [
    /\bgenerate\s+(an?\s+)?image\b/,
    /\bgenerate\s+(an?\s+)?picture\b/,
    /\bgenerate\s+(an?\s+)?photo\b/,
    /\bgenerate\s+(an?\s+)?illustration\b/,
    /\bgenerate\s+(an?\s+)?icon\b/,
    /\bcreate\s+(an?\s+)?image\b/,
    /\bcreate\s+(an?\s+)?picture\b/,
    /\bcreate\s+(an?\s+)?photo\b/,
    /\bcreate\s+(an?\s+)?illustration\b/,
    /\bdraw\s+(me\s+)?(an?\s+)?/,
    /\bsketch\s+(me\s+)?(an?\s+)?/,
    /\brender\s+(me\s+)?(an?\s+)?/,
    /\bmake\s+(me\s+)?(an?\s+)?image\b/,
    /\bmake\s+(me\s+)?(an?\s+)?picture\b/,
    /\bmake\s+(me\s+)?(an?\s+)?photo\b/,
    /\bpaint\s+(me\s+)?(an?\s+)?/,
    /\bdepict\s+/,
    /\bvisualize\s+/,
    /\bshow\s+me\s+(?:an?\s+)?(?:image|picture|photo|illustration)\b/,
    /\bproduce\s+(an?\s+)?image\b/,
    /\bdesign\s+(an?\s+)?(?:image|logo|icon|banner|poster)\b/,
    /\billustrate\s+/,
    /\bpicture\s+of\b/,
    /\bimage\s+of\b/,
    /\bphoto\s+of\b/,
    /\bsnap\s+(?:a\s+)?(?:picture|photo)\b/,
    /\bcapture\s+(an?\s+)?(?:image|picture|photo)\b/,
  ]
  if (imagePatterns.some(p => p.test(q))) return 'generate_image'

  // ── Weather ──
  const weatherPatterns = [
    /\bweather\s+(?:in|at|for|today|tomorrow|forecast)\b/,
    /\bwhat(?:'s|\s+is)\s+the\s+weather\b/,
    /\btemperature\s+(?:in|at|for)\b/,
    /\bforecast\s+(?:in|at|for)\b/,
    /\bis\s+it\s+(?:raining|sunny|cold|hot|warm)\s+(?:in|at)\b/,
    /\bhow(?:'s|\s+is)\s+the\s+weather\b/,
  ]
  if (weatherPatterns.some(p => p.test(q))) return 'get_weather'

  // ── Deploy ──
  const deployPatterns = [
    /\bdeploy\s+(?:this|my|the|a|an)?\s*(?:site|website|page|app|project)\b/,
    /\bpublish\s+(?:this|my|the|a|an)?\s*(?:site|website|page|project)\b/,
    /\bhost\s+(?:this|my|the|a|an)?\s*(?:site|website|page|project)\b/,
    /\bput\s+(?:this|my|the)?\s*(?:site|website)\s+online\b/,
    /\bmake\s+(?:this|my|a)?\s*(?:site|website)\s+live\b/,
  ]
  if (deployPatterns.some(p => p.test(q))) return 'deploy_site'

  // ── QR Code ──
  if (/\b(?:generate|create|make)\s+(?:an?\s+)?(?:qr|qr\s+code)\b/i.test(q)) return 'generate_qr_code'

  // ── Exchange rate ──
  if (/\b(?:exchange|conversion|convert)\s+rate\b/i.test(q) && /\b(?:usd|naira|eur|gbp|yen|rand|cedi|shilling)\b/i.test(q)) return 'get_exchange_rate'

  // ── Crypto price ──
  if (/\b(?:price|value|cost)\s+of\s+(?:bitcoin|btc|ethereum|eth|solana|sol)\b/i.test(q)) return 'get_crypto_price'

  // ── Map ──
  if (/\b(?:generate|create|make|show)\s+(?:an?\s+)?(?:map|location\s+map)\s+(?:of|for)\b/i.test(q)) return 'generate_map'

  // ── Color palette ──
  if (/\b(?:color|colour)\s+palette\b/i.test(q)) return 'color_palette'

  return null
}

export { toolDefs, toolHandlers, TOOL_INSTRUCTIONS, performSearch, readUrl, filterToolsByQuery, detectRequiredTool }
