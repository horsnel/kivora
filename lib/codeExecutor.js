...
//   2. Docker   — self-hosted Docker Engine API, any language, persistent containers
//   3. E2B      — fastest ephemeral (150ms cold start), Jupyter-based, multi-language
//   4. Kaggle   — free GPU (T4/P100), TPU v3-8, API key auth
//   5. Colab    — more GPU types (T4/L4/A100/H100), TPUs, needs OAuth token
//   6. Codespaces — full IDE, any language, persistent, GitHub Actions execution
//
// Decision logic:
//   - Needs GPU/TPU?  → Daytona (H100/RTX Pro) or Kaggle (free T4) or Colab (premium GPUs)
//   - Quick CPU task?  → Daytona (if available) or E2B (fastest ephemeral) or Docker
//   - Full dev env?    → Daytona (persistent sandboxes) or Docker or Codespaces
//   - Self-hosted?     → Docker (most universal, runs on any VPS)
//   - Fallback:        → Daytona → Docker → E2B → Kaggle → Colab → Codespaces
...
export function selectBackend(requirements = {}) {
  ...
  if (preferredPlatform) {
    ...
    const aliases = {
      ...
      'docker': 'docker', 'docker-sandbox': 'docker', 'docker-engine': 'docker',
      'e2b': 'e2b',
  ...
  // Full dev environment or persistent sandbox needed → Daytona first, then Docker, then Codespaces
  ...
  // Large memory needed → Daytona (up to 8GB default, custom), Docker (customizable), Codespaces (up to 64GB)
  if (needsLargeMemory) {
    if (available.daytona) return 'daytona'
    if (available.docker) return 'docker'
    if (available.codespaces) return 'codespaces'
  ...
  // Multi-language → Daytona (any language), Docker (any language), E2B (Jupyter), or Codespaces
  ...
  // Default: Daytona (if available) or Docker or E2B for CPU tasks
  if (available.daytona) return 'daytona'
  if (available.docker) return 'docker'
  if (available.e2b) return 'e2b'
  ...
  try {
    switch (backend) {
      ...
      case 'docker': {
        ...
        const result = await dockerSandbox.runDockerCode(code, {
          language,
          timeout,
        })
        return {
...
function getFallbackChain(failedBackend) {
  const chains = {
    'daytona': ['docker', 'e2b', 'kaggle', 'colab', 'codespaces'],
    'docker': ['daytona', 'e2b', 'kaggle', 'colab', 'codespaces'],
    'e2b': ['daytona', 'docker', 'kaggle', 'colab', 'codespaces'],
    'kaggle': ['daytona', 'docker', 'colab', 'e2b', 'codespaces'],
    'colab': ['daytona', 'docker', 'kaggle', 'e2b', 'codespaces'],
    'codespaces': ['daytona', 'docker', 'e2b', 'kaggle', 'colab'],
  }
  return chains[failedBackend] || ['daytona', 'docker', 'e2b', 'kaggle', 'colab', 'codespaces']
}
...
export function getBackendComparison() {
  return [
    ...
    },
    {
      id: 'docker',
      name: 'Docker Sandbox',
      description: 'Self-hosted Docker Engine API — any language, persistent containers, full filesystem, customizable resources',
      bestFor: 'Self-hosted sandboxes, any VPS, Docker Desktop, full control over resources and images',
      gpu: false,
      coldStart: '~1-3s (local), 5-15s (remote pull)',
      languages: ['Any'],
      auth: 'DOCKER_HOST_URL',
