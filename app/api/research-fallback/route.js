...
2. Use exactly 5 markdown tables distributed across sections: 1 in Context (key data), 1 in Analysis (CONSOLIDATED comparison — do NOT add one per sub-theme), 1 in Comparison (options matrix), 1 in Implications (action items by stakeholder), 1 anywhere else as needed. Each table must carry information that bullets cannot.
3. Cite claims with [N] matching source numbers. Only cite a source you actually read.
4. Confidence language: "established" (2+ sources), "likely" (one source + reasoning), "uncertain" (speculative).
...
## Analysis
3-5 thematic subsections. Use **bold subheading** for each theme, followed by 2-3 short paragraphs. Challenge weak claims. Note contradictions between sources. End the section with 1 CONSOLIDATED comparison table that covers all themes together (do NOT add one table per sub-theme).
...
## Analysis
3-5 thematic subsections. Use **bold subheading** for each theme, followed by 2-3 short paragraphs. Flag uncertainties explicitly. End the section with 1 CONSOLIDATED comparison table that covers all themes together (do NOT add one table per sub-theme).
...
export async function POST(req) {
  ...
  try {
    ...
    const { report: rawReport, followups } = extractFollowups(llmResult.report)
    // Strip any trailing "Sources:" section the model appended despite
    // instructions not to. The Sources tab in the UI shows them separately.
    const report = stripSourcesSection(rawReport)
