...
export default function ResearchClient() {
  ...
  function renderReportTile() {
    ...
    const reportContent = activeResearch?.content || ''
    const reportTitle = activeResearch?.title || activeResearch?.query || ''
    const followups = activeResearch?.followups || []

    // Loading state — spinner + "Researching..." + time estimate
    if (isResearching && !reportText) {
      return (
        <div className="bg-[#111111] border border-[#1a1a1a] rounded-xl overflow-hidden flex flex-col tile-fade-in">
          <div className="flex-1 flex flex-col items-center justify-center py-16">
            <div className="w-8 h-8 border-2 border-[#262626] border-t-red-500 rounded-full animate-spin mb-4" />
            <p className="text-sm text-[#737373]">Researching...</p>
            <p className="text-xs text-[#404040] mt-1">This takes 15–30 seconds</p>
          </div>
        </div>
      )
    }
    ...
    return (
      <div className="bg-[#111111] border border-[#1a1a1a] rounded-xl overflow-hidden flex flex-col tile-fade-in">
        {/* Report header: "Research Report" label + title + Copy button */}
        <div className="px-5 pt-5 pb-3 shrink-0">
          <div className="flex items-start justify-between mb-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-[11px] text-[#525252] hover:text-white transition-colors shrink-0 ml-4 mt-1"
            >
        ...
        {/* Report body — dangerouslySetInnerHTML from API HTML content, or markdown fallback */}
        <div
          ref={reportRef}
          className="flex-1 overflow-y-auto overscroll-behavior-contain max-h-96 lg:max-h-[calc(100vh-320px)] custom-scrollbar px-5 pb-4"
        >
          {reportContent ? (
            <div
              className="prose-kivora"
        ...
        {/* Follow-up Questions */}
        {!isResearching && followups.length > 0 && (
          <div className="border-t border-[#1a1a1a] px-5 py-4">
            <h3 className="text-[10px] font-semibold text-[#525252] uppercase tracking-widest mb-3">
              Follow-up Questions
            </h3>
            <div className="space-y-2">
              {followups.map((q, i) => (
                <button
                  key={i}
                  onClick={() => { setInput(q); startResearch(q, mode) }}
                  className="w-full text-left bg-[#0f0f0f] hover:bg-[#141414] border border-[#1a1a1a] hover:border-[#262626] rounded-lg px-3.5 py-2.5 text-xs text-[#737373] hover:text-white transition-all duration-200 flex items-center justify-between group"
                >
                  <span className="leading-relaxed">{q}</span>
                  <svg width="12" height="12" viewBox="0 0 12 12" fill="none" className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity ml-2">
  ...
  return (
    <div className="h-dvh flex bg-[#0a0a0a] text-white overflow-hidden">
      ...
      <style jsx>{`
        ...
        /* Prose styling for dangerouslySetInnerHTML content from API */
        .prose-kivora h1 {
          font-size: 1.25rem;
          font-weight: 700;
          color: #ffffff;
