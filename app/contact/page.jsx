...
export default function ContactPage() {
  ...
  function validateField(k, v) {
    setErrors(prev => {
      const next = { ...prev }
      if (k === 'email') {
        if (!v?.trim()) next.email = t('contact.validation.email_required')
        else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v)) next.email = t('contact.validation.email_invalid')
        else delete next.email
      }
      if (k === 'message') {
        if (!v?.trim()) next.message = t('contact.validation.message_required')
        else if (v.trim().length < 10) next.message = t('contact.validation.message_short')
  ...
  function hasErrors() {
    ...
    else if (form.message.trim().length < 10) e.message = t('contact.validation.message_short')
    setErrors(e)
  ...
  const errorInputClass = "w-full bg-[#0d0d0d] border border-red-900/60 rounded-xl px-4 py-3 text-sm text-white placeholder-[#404040] transition-all duration-200 focus:outline-none"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div>
            ...
            <div className="space-y-5">
              {[
                { label: t('contact.general'),    email: 'hello@kivora.com',   desc: t('contact.general_desc') },
                { label: t('contact.privacy'),    email: 'privacy@kivora.com', desc: t('contact.privacy_desc') },
                { label: t('contact.legal'),      email: 'legal@kivora.com',   desc: t('contact.legal_desc') },
                { label: t('contact.press'),      email: 'press@kivora.com',   desc: t('contact.press_desc') },
              ].map(c => (
          ...
          <div>
            {status === 'success' ? (
              ...
              <div className="bg-[#141414] rounded-xl p-6 space-y-4">
                ...
                {status === 'error' && !errors.form && <div className="bg-red-950/30 border border-red-900/40 rounded-xl px-4 py-2.5 text-xs text-red-400">{t('contact.error')}</div>}
                <button
                  onClick={submit}
                ...
                >
                  {status === 'loading' ? <><IconSpinner size={14} /> {t('contact.sending')}</> : t('contact.send')}
                </button>
              </div>
            )}
          </div>
